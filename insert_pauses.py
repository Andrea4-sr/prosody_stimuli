...
def insert_pauses(path, pair, break_second_to_onset, break_second_to_offset, correct, outdir):
    ...
    beg = stimuli[:break_second_to_onset].append(pause)
    end = stimuli[break_second_to_offset:]
...
if __name__ == '__main__':
    ...
    #print(chosen_pairs_list)
    paths_dict = dictionary_paths_stimuli(chosen_pairs_list)
    #print(paths_dict)

    for pair, break_second_to_onset, break_second_to_offset, correct in zip(chosen_pairs_df.Pair_Index, chosen_pairs_df.Second_Break_to_Onset, chosen_pairs_df.Second_Break_to_Offset chosen_pairs_df.Correct):
        for paths, num_correct in paths_dict.items():
            if pair == num_correct[0] and correct == num_correct[1]:
                ...
                insert_pauses(paths, pair, break_second_to_onset, break_second_to_offset, correct, args.outdir)

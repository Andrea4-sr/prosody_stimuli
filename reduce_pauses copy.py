...
from create_initial_stimuli_dataset import stim
...
def extract_details_for_crossfading(stim):
    pauses=[]
    for i, (audio, pause_onset, pause_offset, pause_duration) in enumerate(zip(stim['Audio File'], stim['Pause onset'], stim['Pause offset'], stim['Pause Duration'])):
...
def crossfade_wav_files_per_pause(dictionary_of_pauses_per_stimuli):
    for i, (audio, pauses) in enumerate(dictionary_of_pauses_per_stimuli.items()):
        print(audio)
        new_path = '/Users/andreasantos/Documents/Andrea/Studies/Universities/UZH:ETH/Internships/L\'ENS-CoML/Scripts/Stimuli_Pairs/Automatic/Final/'+audio[-12:-4]+'-faded.wav'
...
def main():
    dictionary_of_pauses_per_stimuli = extract_details_for_crossfading(stim)
    crossfade_wav_files_per_pause(dictionary_of_pauses_per_stimuli)
